...
{
    std::unique_ptr<MultiScalarField2D<T> > vorticity = computeVorticity(velocity);
    std::unique_ptr<MultiScalarField2D<T> > enstrophy =
...
template<typename T, template<typename U> class Descriptor>
std::unique_ptr<MultiScalarField2D<T> > solvePressureGS(MultiBlockLattice2D<T,Descriptor>& lattice){
    ...
    // initial pressure
    std::unique_ptr<MultiScalarField2D<T> > initialValue = multiply( DESCRIPTOR<T>::cs2, *add((T)-1., *density) );
    // the result holder
    std::unique_ptr<MultiScalarField2D<T> > result = multiply( DESCRIPTOR<T>::cs2, *add((T)-1., *density) );
    // right hand side of the equation
    std::unique_ptr<MultiScalarField2D<T> > rhs = computePoissonRHS(*velocity);  ;
...
int main(int argc, char* argv[]) {
    ...
    if (computePoisson){                              
        ...
        std::unique_ptr<MultiScalarField2D<T> > press = solvePressureGS(dummy);
        ...
        for (plint iLevel=1; iLevel<numLevel; ++iLevel){
            ...
            std::unique_ptr<MultiScalarField2D<T> > p = solvePressureGS(dummyLattice);
        }
    }
