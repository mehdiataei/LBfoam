        ...
        BlockStatistics& statistics )
{
    ...
    T uSqr = 0.;
    if (rand() < RAND_MAX / 4) {
        uSqr = dynamicsTemplates<T,Descriptor>::bgk_ma2_collision(cell, rhoBar, j, this->getOmega());
    }
    else {
...
template<typename T, template<typename U> class Descriptor>
T StochasticDynamics<T,Descriptor>::computeEquilibrium(plint iPop, T rhoBar, Array<T,Descriptor<T>::d> const& j,
                                                T jSqr, T thetaBar) const
{
    T invRho = Descriptor<T>::invRho(rhoBar);
    return dynamicsTemplates<T,Descriptor>::bgk_ma2_equilibrium(iPop, rhoBar, invRho, j, jSqr);
...
void CompleteRegularizedTRTdynamics<T,Descriptor>::setParameter(plint whichParameter, T value) {
    switch (whichParameter) {
        case dynamicParams::omega_shear     : this->setOmega(value); break;
        case dynamicParams::psi             : setPsi(value);
    };
}
