...
template<typename T, template<typename U> class Descriptor>
LocalViscositySpongeZone3D<T,Descriptor>::LocalViscositySpongeZone3D(Box3D globalDomain_, T bulkOmega_,
        Array<plint,6> const& numSpongeCells_)
...
template<typename T, template<typename U> class Descriptor>
void LocalViscositySpongeZone3D<T,Descriptor>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{
    PLB_ASSERT(blocks.size() == 1);
    BlockLattice3D<T,Descriptor> *lattice = dynamic_cast<BlockLattice3D<T,Descriptor>*>(blocks[0]);
    PLB_ASSERT(lattice);
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            ...
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                ...
                if (numSpongeCells[0] > 0 && i>=x0 && i <= spongePositions[0]) {
                    spongeFunction *= (T) 0.5 * ((T) 1 - std::cos(pi * (T) (i-x0) * invNumSpongeCells[0]));
                    modifyOmega = true;
                }
                ...
                if (numSpongeCells[2] > 0 && j>=y0 && j <= spongePositions[2]) {
                    spongeFunction *= (T) 0.5 * ((T) 1 - std::cos(pi * (T) (j-y0) * invNumSpongeCells[2]));
                    modifyOmega = true;
                }
                ...
                if (numSpongeCells[4] > 0 && k>=z0 && k <= spongePositions[4]) {
                    spongeFunction *= (T) 0.5 * ((T) 1 - std::cos(pi * (T) (k-z0) * invNumSpongeCells[4]));
                    modifyOmega = true;
                }
