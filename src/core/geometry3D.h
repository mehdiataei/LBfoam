...
struct Box3D {
	...
	plint getMaxWidth() const
	{
		return std::max(std::max(getNx(), getNy()), getNz());
	}
...
struct Dot3D {
	...
	template<typename T>
	Array<T,3> to_plbArray() const
	{
		return Array<T,3>((T)x,(T)y,(T)z);
	}
...
struct Cuboid {
	...
	T& x0()
	{
		return lowerLeftCorner[0];
	}
	T& y0()
	{
		return lowerLeftCorner[1];
	}
	T& z0()
	{
		return lowerLeftCorner[2];
	}
	T& x1()
	{
		return upperRightCorner[0];
	}
	T& y1()
	{
		return upperRightCorner[1];
	}
	T& z1()
	{
		return upperRightCorner[2];
	}
