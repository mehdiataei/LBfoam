...
{
    // Make sure the new domain is at least one node large.
...
void Units3D::extend1()
{
    physDomain_.lowerLeftCorner[0] -= 0.5*dx_;
    physDomain_.lowerLeftCorner[1] -= 0.5*dx_;
    physDomain_.lowerLeftCorner[2] -= 0.5*dx_;

    physDomain_.upperRightCorner[0] += 0.5*dx_;
    physDomain_.upperRightCorner[1] += 0.5*dx_;
    physDomain_.upperRightCorner[2] += 0.5*dx_;

    lbDomain_ = Box3D (
            0, lbDomain_.getNx(), 0, lbDomain_.getNy(), 0, lbDomain_.getNz() );
}

void Units3D::shift(double dx0, double dy0, double dz0)
{
    physDomain_.lowerLeftCorner[0] += dx0*dx_;
    physDomain_.lowerLeftCorner[1] += dy0*dx_;
    physDomain_.lowerLeftCorner[2] += dz0*dx_;

    physDomain_.upperRightCorner[0] += dx0*dx_;
    physDomain_.upperRightCorner[1] += dy0*dx_;
    physDomain_.upperRightCorner[2] += dz0*dx_;
}
