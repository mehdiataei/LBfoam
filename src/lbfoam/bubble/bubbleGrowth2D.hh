...
#ifndef BUBBLE_GROWTH_2D_HH
#define BUBBLE_GROWTH_2D_HH
...
template <typename T>
BubbleGrowth2D<T>::BubbleGrowth2D(MultiBlock2D &templ)
    : bubbleAnalysisContainer(
          createContainerBlock(templ, new BubbleAnalysisData2D())),
      bubbleCorrelationContainer(
          createContainerBlock(templ, new BubbleCorrelationData2D())),
      bubbleRemapContainer(
          createContainerBlock(templ, new BubbleRemapData2D())),
      mpiData(templ),
...
template <typename T>
BubbleGrowth2D<T>::~BubbleGrowth2D() {
...
template <typename T>
void BubbleGrowth2D<T>::transition(BubbleTracking2D &bubbleTracking,
                                   plint iterationStep, T temperature, T R_s,
                                   T p_sat, T dx, T fluidRho,
                                   T newBubbleVolumeCorrection,
                                   bool entrapBubbles,
                                   plint numRememberedVolumes) {
  matchAndRemapBubbles(bubbleTracking, *oldTagMatrix,
                       *bubbleTracking.getTagMatrix(),
                       newBubbleVolumeCorrection, iterationStep, entrapBubbles,
                       numRememberedVolumes);

  scaleAndIncreaseDensity(temperature, R_s, p_sat, dx, fluidRho);

  // Get the tag-matrix from bubbleTracking, and store it as internal
  // oldTagMatrix for next iteration.
  MultiScalarField2D<plint> *newTagMatrix = bubbleTracking.getTagMatrix();
  bubbleTracking.setTagMatrix(oldTagMatrix);
  oldTagMatrix = newTagMatrix;
}

template <typename T>
void BubbleGrowth2D<T>::updateBubblePressure(
    MultiScalarField2D<T> &outsideDensity, T rhoEmpty, T alpha, T beta,
    T gamma) {
  applyProcessingFunctional(
      new UpdateBubblePressure2D<T>(bubbles, rhoEmpty, alpha, beta, gamma),
      oldTagMatrix->getBoundingBox(), *oldTagMatrix, outsideDensity);
}

template <typename T>
void BubbleGrowth2D<T>::updateBubbleGrowth(
    MultiScalarField2D<T> &outsideDensity, T rhoEmpty, T alpha, T beta,
    T gamma) {
  applyProcessingFunctional(
      new UpdateBubbleGrowth2D<T>(bubbles, rhoEmpty, alpha, beta, gamma),
      oldTagMatrix->getBoundingBox(), *oldTagMatrix, outsideDensity);
}

template <typename T>
void BubbleGrowth2D<T>::freeze() {
...
template <typename T>
void BubbleGrowth2D<T>::freezeLargestBubble() {
  typename std::map<plint, BubbleInfo2D>::iterator it;
...
template <typename T>
void BubbleGrowth2D<T>::timeHistoryLog(std::string fName) {
  plb_ofstream ofile(fName.c_str());
  std::map<plint,
           std::pair<std::vector<plint>, std::vector<plint>>>::const_iterator
...
template <typename T>
void BubbleGrowth2D<T>::fullBubbleLog(std::string fName) {
  plb_ofstream ofile(fName.c_str());
...
template <typename T>
void BubbleGrowth2D<T>::matchAndRemapBubbles(
    BubbleTracking2D &bubbleTracking, MultiScalarField2D<plint> &tagMatrix1,
    MultiScalarField2D<plint> &tagMatrix2, T newBubbleVolumeCorrection,
    plint iterationStep, bool entrapBubbles, plint numRememberedVolumes) {
  pluint numBubbles = bubbleTracking.numBubbles();
  // The following data processor creates a correspondance between new bubbles
  // and all overlapping old bubbles. This is the basis of the upcoming
  // contrived algorithm.
  std::vector<std::vector<plint>> newToAllOldMap;
  correlateBubbleIds(tagMatrix1, tagMatrix2, newToAllOldMap, numBubbles);

  // First, the map is inverted, to get a correspondence between old bubbles and
  // all overlapping new bubbles.
  std::map<plint, std::vector<plint>> oldToAllNewMap;
  typename std::map<plint, BubbleInfo2D>::const_iterator itOld =
      bubbles.begin();
  ...
  for (plint newID = 0; newID < (plint)newToAllOldMap.size(); ++newID) {
    for (pluint i = 0; i < newToAllOldMap[newID].size(); ++i) {
      ...
      typename std::map<plint, std::vector<plint>>::iterator it =
          oldToAllNewMap.find(oldID);
      PLB_ASSERT(it != oldToAllNewMap.end());
      it->second.push_back(newID);
    }
  }

  // Then, a transition pattern between groups of old and new bubbles is
  // determined.
  ...
  updateBubbleInformation(bubbleTransitions, bubbleTracking.getBubbleVolume(),
                          bubbleTracking.getBubbleDensity(),
                          bubbleTracking.getBubbleDisjoiningPressure(),
  ...
  applyProcessingFunctional(new ApplyTagRemap2D(),
                            bubbleRemapContainer->getBoundingBox(), args2);
}

template <typename T>
void BubbleGrowth2D<T>::updateBubbleInformation(
    std::vector<BubbleTransition2D> &bubbleTransitions,
  ...
  for (pluint i = 0; i < bubbleTransitions.size(); ++i) {
    ...
    computeNewBubbles(oldIDs, newIDs, bubbleVolume, bubbleDensity,
                      bubbleDisjoiningPressure, newBubbleVolumeCorrection,
                      newBubbles, newToFinal, entrapBubbles,
                      numRememberedVolumes);

    updateBubbleLog(bubbleTransitions[i], bubbleVolume, iterationStep,
                    newBubbles, newToFinal);
  }
...
template <typename T>
void BubbleGrowth2D<T>::scaleAndIncreaseDensity(T temperature, T R_s, T p_sat,
                                                T dx, T fluidRho) {
  typename std::map<plint, BubbleInfo2D>::iterator it;
  for (it = bubbles.begin(); it != bubbles.end(); it++) {
    BubbleInfo2D &info = it->second;
    T scaledIncrease = info.getIncrim() * R_s * temperature * fluidRho /
                       (p_sat * info.getReferenceVolume());
    info.increaseDensity(scaledIncrease);
  }
}

template <typename T>
void BubbleGrowth2D<T>::computeNewBubbles(
    std::set<plint> &oldIDs, std::set<plint> &newIDs,
    std::vector<double> const &bubbleVolume,
    std::vector<double> const &bubbleDensity,
    std::vector<double> const &bubbleDisjoiningPressure,
    T newBubbleVolumeCorrection, std::map<plint, BubbleInfo2D> &newBubbles,
    std::map<plint, plint> &newToFinal, bool entrapBubbles,
    plint numRememberedVolumes) {
  // In this function there is code which implements two additional ideas:
  // 1.] Bubbles with a volume smaller than the cutoff-volume get automatically
  // frozen.
  //     The cutoff value is given by the getCutoffVolume() function (this
  //     feature can be deactivated if the cutoff value is set to 0).
  // 2.] In the case of "bubble entrapment", if an original bubble is frozen and
  // it splits
  //     into new bubbles, then only the largest of the new bubbles remains
  //     frozen.
  ...
  if (oldIDs.empty()) {
    ...
    newBubbles[nextBubbleID] =
        BubbleInfo2D(bubbleVolume[newID] * newBubbleVolumeCorrection,
                     numRememberedVolumes, (double)1.);
    // If a newly created bubble is smaller than the cutoff value, it gets
  ...
  // All other cases: normal transition, splitting, merging.
    ...
    // First check the old bubbles: their total initial volume will be
    // distributed as initial volume of the new bubbles. If one of the old
    // bubbles is frozen, then so are the new bubbles. And while we're at it,
    // also close the history record for these old bubbles.
    T totalReferenceVolume = T();
    bool isFrozen = false;
    std::set<plint>::const_iterator itOld = oldIDs.begin();
    for (; itOld != oldIDs.end(); ++itOld) {
      plint oldID = *itOld;
      typename std::map<plint, BubbleInfo2D>::const_iterator it =
          bubbles.find(oldID);
    ...
    if (oldIDs.size() == 1) {
      if (newIDs.size() == 1) {
        ...
        typename std::map<plint, BubbleInfo2D>::const_iterator it =
            bubbles.find(oldID);
        density.insert(
            std::pair<plint, T>(newID, it->second.getCurrentDensity()));
      } else {
        // Splitting
        std::set<plint>::const_iterator itOld = oldIDs.begin();
        std::set<plint>::const_iterator itNew = newIDs.begin();
      ...
      for (; itOld != oldIDs.end(); ++itOld) {
        ...
        totalDensityTimesRefVolume +=
            it->second.getCurrentDensity() * it->second.getReferenceVolume();
      }
    ...
    for (; itNew != newIDs.end(); ++itNew) {
      ...
      BubbleInfo2D nextBubble(newInitialVolume, numRememberedVolumes,
                              newDensity);
      ...
      // LB_ASSERT(nextBubble.getVolume() >= 0)
      if (nextBubble.getVolume() < cutoffVolume) {
        nextBubble.setVolume(1.);
        nextBubble.freeze();
      }
      ...
      if (!entrapBubbles) {
        ...
        if (isFrozen && itNew == itLargest) {
          nextBubble.setDensity(1.);
          nextBubble.freeze();
        } else if (isFrozen && itNew != itLargest) {
          nextBubble.unfreeze();
          nextBubble.setDensity(1.);
          nextBubble.setReferenceVolume(bubbleVolume[newID] *
                                        newBubbleVolumeCorrection);
        }
      }
...
template <typename T>
void BubbleGrowth2D<T>::updateBubbleLog(
    BubbleTransition2D &bubbleTransition,
    std::vector<double> const &bubbleVolume, plint iterationStep,
    std::map<plint, BubbleInfo2D> &newBubbles,
    std::map<plint, plint> &newToFinal) {
  ...
  for (std::set<plint>::iterator it = newIDs.begin(); it != newIDs.end();
       ++it) {
    save_newIDs.insert(newToFinal[*it]);
  }
  save_newIDs.swap(newIDs);

  // For purposes of history, keep a full record of the bubble (start off the
  // record). Furthermore, keep a time-record of the fact that a bubble was
  // created.
  //
  // A bubble is created from nothing.
  if (oldIDs.empty()) {
    ...
    FullBubbleRecord2D nextBubbleRecord(bubbleVolume[newID], iterationStep);
...
template <typename T>
void BubbleGrowth2D<T>::correlateBubbleIds(
  ...
  for (pluint i = 0; i < localIds.size(); ++i) {
    ...
    BubbleCorrelationData2D *pData =
        dynamic_cast<BubbleCorrelationData2D *>(atomicDataContainer.getData());
    PLB_ASSERT(pData);
  ...
  // The number of iterations in the coming loop will be N+1, where N is the
  // maximum number of bubbles from the physical time step t-1 that coalesced to
  // form a bubble at time t. Very often, no coalescence occurs and N is just 1.
  ...
  while (!hasConverged) {
    ...
    applyProcessingFunctional(new CorrelateBubbleIds2D<T>,
                              bubbleCorrelationContainer->getBoundingBox(),
                              args);

    std::vector<plint> newToOldMap(numBubbles, maxInt);

    // Apply the MIN reduction to all atomic-blocks that are on the current MPI
    // thread.
    for (pluint i = 0; i < localIds.size(); ++i) {
      plint id = localIds[i];
      AtomicContainerBlock2D &atomicDataContainer =
          bubbleCorrelationContainer->getComponent(id);
      BubbleCorrelationData2D *pData = dynamic_cast<BubbleCorrelationData2D *>(
          atomicDataContainer.getData());
      PLB_ASSERT(pData);
      BubbleCorrelationData2D &data = *pData;
    ...
    // If new bubbles where found, add them to the return value. If not, abort
    // the iterations.
    ...
    // Assign the result of the global reduction to all local atomic-blocks, to
    ...
    for (pluint i = 0; i < localIds.size(); ++i) {
      ...
      // Advance by one iteration: assign the globally reduced data to
      // newToOldMap0
      // ("the previous iteration").
      data.newToOldMap0.assign(newToOldMap.begin(), newToOldMap.end());
      // Prepare newToOldMap1 for the next iteration, and especially for the
      // coming MIN reduction, by initializing to maxInt.
      std::fill(data.newToOldMap1.begin(), data.newToOldMap1.end(), maxInt);
    }
  }
}
...
void CorrelateBubbleIds2D<T>::processGenericBlocks(
  ...
  BubbleCorrelationData2D *pData =
      dynamic_cast<BubbleCorrelationData2D *>(dataBlock.getData());
  PLB_ASSERT(pData);
  BubbleCorrelationData2D &data = *pData;
    ...
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      ...
      if (newBubbleTag >= 0) {
        ...
        if (oldBubbleTag >= 0) {
          ...
          // At this step of the iteration (corresponding to a call to the
          // present data processor), we're only targeting tags that are larger
          // than the ones from the previous step.
...
template <typename T>
UpdateBubblePressure2D<T>::UpdateBubblePressure2D(
    std::map<plint, BubbleInfo2D> const &bubbles_, T rho0_, T alpha_, T beta_,
    T gamma_)
    : bubbles(bubbles_),
      rho0(rho0_),
      alpha(alpha_),
...
template <typename T>
void UpdateBubblePressure2D<T>::process(Box2D domain,
                                        ScalarField2D<plint> &tags,
      ...
      if (tag >= 0) {
        ...
        if (!info.isFrozen()) {
          ...
          if (alpha < (T)0 || beta < (T)0) {
            ...
            rho = rho0 *
                  (1.0 + alpha * std::pow((T)(1.0 - 1.0 / volumeRatio), beta));
          }
...
template <typename T>
void BubbleGrowth2D<T>::computeBubbleTransitions(
  ...
  std::map<plint, std::vector<plint>>::const_iterator it =
      oldToAllNewMap.begin();
...
/************ Class UpdateBubbleGrowth2D ******************************** */

template <typename T>
UpdateBubbleGrowth2D<T>::UpdateBubbleGrowth2D(
    std::map<plint, BubbleInfo2D> const &bubbles_, T rho0_, T alpha_, T beta_,
    T gamma_)
    : bubbles(bubbles_),
      rho0(rho0_),
...
template <typename T>
UpdateBubbleGrowth2D<T> *UpdateBubbleGrowth2D<T>::clone() const {
  return new UpdateBubbleGrowth2D<T>(*this);
}

template <typename T>
void UpdateBubbleGrowth2D<T>::process(Box2D domain, ScalarField2D<plint> &tags,
                                      ScalarField2D<T> &density) {
  Dot2D ofs = computeRelativeDisplacement(tags, density);
  for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      ...
      if (tag >= 0) {
        ...
        if (!info.isFrozen()) {
          ...
          if (alpha < (T)0 || beta < (T)0) {
            ...
            // pcout << "from getCurrentDensity: " << info.getCurrentDensity()
            // << " id:  " << tag << std::endl;
          } else {
            // Model: rho = rho0 * [1 + alpha * (1 - V/V0)^beta]
            rho = info.getCurrentDensity() *
                  (1.0 + alpha * std::pow((T)(1.0 - 1.0 / volumeRatio), beta));
          }
        }
        density.get(iX + ofs.x, iY + ofs.y) = rho;
