...
#ifndef BUBBLE_GROWTH_3D_H
#define BUBBLE_GROWTH_3D_H
...
class BubbleInfo3D;
struct FullBubbleRecord3D;

template <typename T>
class BubbleGrowth3D {
 public:
  BubbleGrowth3D(MultiBlock3D &templ);
  ~BubbleGrowth3D();
  void transition(BubbleTracking3D &bubbleTracking, plint iterationStep,
  ...
  void updateBubbleGrowth(MultiScalarField3D<T> &outsideDensity, T rhoEmpty,
                          T alpha = (T)-1, T beta = (T)-1, T gamma = (T)1);
  void scaleAndIncreaseDensity(T temperature, T R_s, T p_sat, T dx, T fluidRho);
  MultiScalarField3D<plint> *getOldTagMatrix() { return oldTagMatrix; }
  void freeze();
  void freezeLargestBubble();
  void timeHistoryLog(std::string fName);
  void fullBubbleLog(std::string fName);
  std::map<plint, lbfoam::BubbleInfo3D> const &getBubbles() const {
    return bubbles;
  }
 ...
 private:
  ...
  void matchAndRemapBubbles(BubbleTracking3D &bubbleTracking,
  ...
  // attributed).
  void correlateBubbleIds(MultiScalarField3D<plint> &tagMatrix1,
                          MultiScalarField3D<plint> &tagMatrix2,
  ...
  void computeNewBubbles(std::set<plint> &oldIDs, std::set<plint> &newIDs,
                         ...
                         plint numRememberedVolumes);
  void updateBubbleLog(BubbleTransition3D &bubbleTransition,
                       std::vector<double> const &bubbleVolume,
                       plint iterationStep,
                       std::map<plint, BubbleInfo3D> &newBubbles,
 ...
 private:
  BubbleGrowth3D(BubbleGrowth3D<T> const &rhs) { PLB_ASSERT(false); }
  BubbleGrowth3D<T> &operator=(BubbleGrowth3D<T> const &rhs) {
    PLB_ASSERT(false);
    return *this;
  }
 ...
 private:
  ...
  std::map<plint, std::pair<std::vector<plint>, std::vector<plint>>>
      timeHistory;
  std::vector<FullBubbleRecord3D> FullBubbleRecord;
  /// IMPORTANT ///
...
class BubbleInfo3D {
 public:
  ...
  BubbleInfo3D(double volume, plint numRememberedVolumes_, double density)
  ...
  double setDisjoiningPressureField() const { return disjoiningPressure; }
...
struct FullBubbleRecord3D {
  FullBubbleRecord3D(double initialVolume_, plint beginIteration_)
...
class UpdateBubblePressure3D : public BoxProcessingFunctional3D_SS<plint, T> {
 ...
 private:
  static T getCutoffDensityRatio() { return 0.0; }
...
template <typename T>
class UpdateBubbleGrowth3D : public BoxProcessingFunctional3D_SS<plint, T> {
 public:
  UpdateBubbleGrowth3D(std::map<plint, BubbleInfo3D> const &bubbles_, T rho0_,
                       T alpha_, T beta_, T gamma_);
  virtual void process(Box3D domain, ScalarField3D<plint> &tags,
                       ScalarField3D<T> &density);
  virtual UpdateBubbleGrowth3D *clone() const;
  virtual void getTypeOfModification(
      std::vector<modif::ModifT> &modified) const {
    modified[0] = modif::nothing;          // tags.
    modified[1] = modif::staticVariables;  // density.
