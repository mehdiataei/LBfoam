...
#ifndef BUBBLE_GROWTH_3D_HH
#define BUBBLE_GROWTH_3D_HH
...
namespace plb {
namespace lbfoam {
/* ************** class BubbleGrowth3D ********************************** */

template <typename T>
BubbleGrowth3D<T>::BubbleGrowth3D(MultiBlock3D &templ)
    : bubbleAnalysisContainer(
          createContainerBlock(templ, new BubbleAnalysisData3D())),
      bubbleCorrelationContainer(
          createContainerBlock(templ, new BubbleCorrelationData3D())),
      bubbleRemapContainer(
          createContainerBlock(templ, new BubbleRemapData3D())),
      mpiData(templ),
...
template <typename T>
BubbleGrowth3D<T>::~BubbleGrowth3D() {
...
template <typename T>
void BubbleGrowth3D<T>::transition(BubbleTracking3D &bubbleTracking,
  ...
  MultiScalarField3D<plint> *newTagMatrix = bubbleTracking.getTagMatrix();
  bubbleTracking.setTagMatrix(oldTagMatrix);
  oldTagMatrix = newTagMatrix;
}

template <typename T>
void BubbleGrowth3D<T>::updateBubblePressure(
    MultiScalarField3D<T> &outsideDensity, T rhoEmpty, T alpha, T beta,
    T gamma) {
...
template <typename T>
void BubbleGrowth3D<T>::updateBubbleGrowth(
    MultiScalarField3D<T> &outsideDensity, T rhoEmpty, T alpha, T beta,
    T gamma) {
  applyProcessingFunctional(
...
template <typename T>
void BubbleGrowth3D<T>::freezeLargestBubble() {
...
template <typename T>
void BubbleGrowth3D<T>::timeHistoryLog(std::string fName) {
...
template <typename T>
void BubbleGrowth3D<T>::fullBubbleLog(std::string fName) {
  plb_ofstream ofile(fName.c_str());
...
template <typename T>
void BubbleGrowth3D<T>::matchAndRemapBubbles(
    BubbleTracking3D &bubbleTracking, MultiScalarField3D<plint> &tagMatrix1,
  ...
  typename std::map<plint, BubbleInfo3D>::const_iterator itOld =
      bubbles.begin();
...
template <typename T>
void BubbleGrowth3D<T>::updateBubbleInformation(
    std::vector<BubbleTransition3D> &bubbleTransitions,
...
template <typename T>
void BubbleGrowth3D<T>::scaleAndIncreaseDensity(T temperature, T R_s, T p_sat,
                                                T dx, T fluidRho) {
...
template <typename T>
void BubbleGrowth3D<T>::computeNewBubbles(
  ...
  if (oldIDs.empty()) {
    ...
    newBubbles[nextBubbleID] =
        BubbleInfo3D(bubbleVolume[newID] * newBubbleVolumeCorrection,
  ...
  // All other cases: normal transition, splitting, merging.
    ...
    for (; itOld != oldIDs.end(); ++itOld) {
      plint oldID = *itOld;
      typename std::map<plint, BubbleInfo3D>::const_iterator it =
          bubbles.find(oldID);
...
template <typename T>
void BubbleGrowth3D<T>::updateBubbleLog(
    BubbleTransition3D &bubbleTransition,
...
template <typename T>
void BubbleGrowth3D<T>::correlateBubbleIds(
    MultiScalarField3D<plint> &tagMatrix1,
    MultiScalarField3D<plint> &tagMatrix2,
  ...
  while (!hasConverged) {
    ...
    applyProcessingFunctional(new CorrelateBubbleIds3D<T>,
    ...
    for (pluint i = 0; i < localIds.size(); ++i) {
      ...
      AtomicContainerBlock3D &atomicDataContainer =
          bubbleCorrelationContainer->getComponent(id);
      BubbleCorrelationData3D *pData = dynamic_cast<BubbleCorrelationData3D *>(
...
void CorrelateBubbleIds3D<T>::processGenericBlocks(
  ...
  BubbleCorrelationData3D *pData =
      dynamic_cast<BubbleCorrelationData3D *>(dataBlock.getData());
  PLB_ASSERT(pData);
  BubbleCorrelationData3D &data = *pData;
  ...
  for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        plint newBubbleTag =
            tagMatrix2.get(iX + tag2Ofs.x, iY + tag2Ofs.y, iZ + tag2Ofs.z);
        if (newBubbleTag >= 0) {
          ...
          if (oldBubbleTag >= 0) {
            ...
            // present data processor), we're only targeting tags that are
            // larger than the ones from the previous step.
...
UpdateBubblePressure3D<T>::UpdateBubblePressure3D(
    std::map<plint, BubbleInfo3D> const &bubbles_, T rho0_, T alpha_, T beta_,
    T gamma_)
    : bubbles(bubbles_),
      rho0(rho0_),
      alpha(alpha_),
...
template <typename T>
void UpdateBubblePressure3D<T>::process(Box3D domain,
                                        ScalarField3D<plint> &tags,
        ...
        if (tag >= 0) {
          ...
          if (!info.isFrozen()) {
            ...
            if (alpha < (T)0 || beta < (T)0) {
              ...
              rho = rho0 * (1.0 + alpha * std::pow((T)(1.0 - 1.0 / volumeRatio),
                                                   beta));
            }
...
template <typename T>
void BubbleGrowth3D<T>::computeBubbleTransitions(
...
template <typename T>
UpdateBubbleGrowth3D<T>::UpdateBubbleGrowth3D(
    std::map<plint, BubbleInfo3D> const &bubbles_, T rho0_, T alpha_, T beta_,
    T gamma_)
    : bubbles(bubbles_),
      rho0(rho0_),
...
template <typename T>
void UpdateBubbleGrowth3D<T>::process(Box3D domain, ScalarField3D<plint> &tags,
                                      ScalarField3D<T> &density) {
  Dot3D ofs = computeRelativeDisplacement(tags, density);
  for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
    for (plint iY = domain.y0; iY <= domain.y1; ++iY)
