...
void VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>::process(
  ...
  const int velOffset =
      TemperatureDescriptor<T>::ExternalField::velocityBeginsAt;
...
template <typename T, template <typename U> class TemperatureDescriptor>
BlockDomain::DomainT
VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>::appliesTo() const {
  return BlockDomain::bulk;
}

template <typename T, template <typename U> class TemperatureDescriptor>
void VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>::
...
void velocityToPassiveAdvDiff(
  ...
  applyProcessingFunctional(
      new VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>(), domain,
...
template <typename T, template <typename U> class TemperatureDescriptor>
BlockDomain::DomainT
N_VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>::appliesTo() const {
  return BlockDomain::bulk;
}

template <typename T, template <typename U> class TemperatureDescriptor>
void N_VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>::
    getTypeOfModification(std::vector<modif::ModifT> &modified) const {
  modified[0] = modif::staticVariables;
  modified[1] = modif::nothing;
}
...
void NVelocityToPassiveAdvDiff(
  ...
  PLB_PRECONDITION(velocity.getNdim() == 3);
  applyProcessingFunctional(
      new N_VelocityToPassiveAdvDiff3D<T, TemperatureDescriptor>(), domain,
      temperature, velocity);
}

template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
LatticeToPassiveAdvDiff3D<
    T, FluidDescriptor, ScalarDescriptor>::LatticeToPassiveAdvDiff3D(T scaling_)
    : scaling(scaling_) {}
...
void LatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>::process(
  ...
  for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        T *u = scalar.get(iX + offset.x, iY + offset.y, iZ + offset.z)
                   .getExternal(velOffset);
        Array<T, 3> velocity;
...
LatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>::clone() const {
  return new LatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>(
      *this);
}

template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
BlockDomain::DomainT LatticeToPassiveAdvDiff3D<
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
void LatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>::
    getTypeOfModification(std::vector<modif::ModifT> &modified) const {
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
void latticeToPassiveAdvDiff(MultiBlockLattice3D<T, FluidDescriptor> &fluid,
                             MultiBlockLattice3D<T, ScalarDescriptor> &scalar,
                             Box3D domain) {
  applyProcessingFunctional(
      new LatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>(),
      domain, fluid, scalar);
}
...
LatticeToPassiveComplRegAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>::
    LatticeToPassiveComplRegAdvDiff3D(T scaling_)
    : scaling(scaling_) {}

template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
void LatticeToPassiveComplRegAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>::
    process(Box3D domain, BlockLattice3D<T, FluidDescriptor> &fluid,
            BlockLattice3D<T, ScalarDescriptor> &scalar) {
  Dot3D offset = computeRelativeDisplacement(fluid, scalar);
    ...
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        ...
        *scalar.get(iX + offset.x, iY + offset.y, iZ + offset.z)
             .getExternal(omegaOffset) =
            fluid.get(iX, iY, iZ).getDynamics().getOmega();
        *scalar.get(iX + offset.x, iY + offset.y, iZ + offset.z)
             .getExternal(rhoBarOffset) = rhoBar;
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
LatticeToPassiveComplRegAdvDiff3D<T, FluidDescriptor, ScalarDescriptor> *
LatticeToPassiveComplRegAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>::clone()
    const {
  return new LatticeToPassiveComplRegAdvDiff3D<T, FluidDescriptor,
                                               ScalarDescriptor>(*this);
}

template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
BlockDomain::DomainT LatticeToPassiveComplRegAdvDiff3D<
    T, FluidDescriptor, ScalarDescriptor>::appliesTo() const {
  return BlockDomain::bulk;
}
...
void latticeToPassiveComplRegAdvDiff(
  ...
  applyProcessingFunctional(
      new LatticeToPassiveComplRegAdvDiff3D<T, FluidDescriptor,
                                            ScalarDescriptor>(),
      domain, fluid, scalar);
}
    ...
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        ...
        if (!util::isZero(fluidOmega)) {
          ...
          T scalarOmega = (T)1 / ((T)1 / scalarOmega0 +
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
TurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>
    *TurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                        ScalarDescriptor>::clone() const {
  return new TurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                                ScalarDescriptor>(*this);
}

template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
BlockDomain::DomainT TurbulentLatticeToPassiveAdvDiff3D<
    T, FluidDescriptor, ScalarDescriptor>::appliesTo() const {
  return BlockDomain::bulk;
}
...
void turbulentLatticeToPassiveAdvDiff(
  ...
  applyProcessingFunctional(
      new TurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                             ScalarDescriptor>(Pr_t),
      domain, fluid, scalar);
}
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
void MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                              ScalarDescriptor>::
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>
    *MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                              ScalarDescriptor>::clone() const {
  return new MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                                      ScalarDescriptor>(*this);
}
...
void maskedTurbulentLatticeToPassiveAdvDiff(
  ...
  args.push_back(&mask);
  applyProcessingFunctional(
      new MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                                   ScalarDescriptor>(Pr_t,
                                                                     whichFlag),
      domain, args);
}
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                           ScalarDescriptor>::
...
template <typename T, template <typename U1> class FluidDescriptor,
          template <typename U2> class ScalarDescriptor>
void N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                                ScalarDescriptor>::
    processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> blocks) {
  PLB_ASSERT(blocks.size() == 3);
  BlockLattice3D<T, FluidDescriptor> *fluid =
...
N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor, ScalarDescriptor>
    *N_MaskedTurbulentLatticeToPassiveAdvDiff3D<
        T, FluidDescriptor, ScalarDescriptor>::clone() const {
  return new N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                                        ScalarDescriptor>(
      *this);
}
...
void NMaskedTurbulentLatticeToPassiveAdvDiff(
  ...
  args.push_back(&mask);
  applyProcessingFunctional(
      new N_MaskedTurbulentLatticeToPassiveAdvDiff3D<T, FluidDescriptor,
                                                     ScalarDescriptor>(
          Pr_t, whichFlag),
      domain, args);
}
...
template <typename T, template <typename U> class Descriptor>
void crystallizeAndAggregate(MultiBlockLattice3D<T, Descriptor> &lattice, T Ncr,
                             T Nag, Box3D domain) {
  applyProcessingFunctional(
...
AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>::
    AdvectionDiffusionFreeSurfaceCoupling3D(
        AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
...
AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor> &
AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>::
operator=(AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                  FS_Descriptor> const &rhs) {
...
void AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>::
    swap(AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                 FS_Descriptor> &rhs) {
    ...
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        ...
        bool tempIsAdiabatic =
            tempBak.get(iX, iY, iZ).getDynamics().getId() == adiabaticId;

        if (flag == fluid && tempIsAdiabatic) {
          tempLattice->attributeDynamics(iX, iY, iZ,
                                         conductiveDynamics->clone());
          ...
          for (plint dx = -d; dx <= d; dx++) {
            ...
            for (plint dy = -d; dy <= d; dy++) {
              ...
              for (plint dz = -d; dz <= d; dz++) {
                ...
                // plint nextFlag =
                // flags->get(i+ofsFlag.x,j+ofsFlag.y,k+ofsFlag.z); if(!(dx==0
                // && dy==0 && dz==0) &&
...
AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>
  ...
  return new AdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                     FS_Descriptor>(*this);
}
...
MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>::
    MaskedAdvectionDiffusionFreeSurfaceCoupling3D(
        MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                      FS_Descriptor> const &rhs)
    : conductiveDynamics(rhs.conductiveDynamics->clone()),
      adiabaticDynamics(rhs.adiabaticDynamics->clone()),
      iniVal(rhs.iniVal),
...
MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor> &
MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>::
operator=(MaskedAdvectionDiffusionFreeSurfaceCoupling3D<
...
template <typename T, template <typename U1> class AD_Descriptor,
          template <typename U2> class FS_Descriptor>
void MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                   FS_Descriptor>::
    swap(MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                       FS_Descriptor> &rhs) {
  std::swap(conductiveDynamics, rhs.conductiveDynamics);
  std::swap(adiabaticDynamics, rhs.adiabaticDynamics);
  std::swap(iniVal, rhs.iniVal);
...
template <typename T, template <typename U1> class AD_Descriptor,
          template <typename U2> class FS_Descriptor>
void MaskedAdvectionDiffusionFreeSurfaceCoupling3D<
    T, AD_Descriptor,
    FS_Descriptor>::processGenericBlocks(Box3D domain,
...
MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                              FS_Descriptor>::clone() const {
  return new MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                           FS_Descriptor>(
      *this);
}
...
template <typename T, template <typename U1> class AD_Descriptor,
          template <typename U2> class FS_Descriptor>
N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                FS_Descriptor>::
...
N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<
    T, AD_Descriptor,
    FS_Descriptor>::~N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D() {
  delete conductiveDynamics;
  delete adiabaticDynamics;
}
...
N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>
    &N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                     FS_Descriptor>::
    operator=(N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<
              T, AD_Descriptor, FS_Descriptor> const &rhs) {
  N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                  FS_Descriptor>(rhs)
      .swap(*this);
...
template <typename T, template <typename U1> class AD_Descriptor,
          template <typename U2> class FS_Descriptor>
void N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                     FS_Descriptor>::
    swap(N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                         FS_Descriptor> &rhs) {
  std::swap(conductiveDynamics, rhs.conductiveDynamics);
  std::swap(adiabaticDynamics, rhs.adiabaticDynamics);
  std::swap(iniVal, rhs.iniVal);
...
template <typename T, template <typename U1> class AD_Descriptor,
          template <typename U2> class FS_Descriptor>
void N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<
    T, AD_Descriptor,
    FS_Descriptor>::processGenericBlocks(Box3D domain,
                                         std::vector<AtomicBlock3D *> blocks) {
  using namespace freeSurfaceFlag3D;
...
N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor, FS_Descriptor>
  ...
  return new N_MaskedAdvectionDiffusionFreeSurfaceCoupling3D<T, AD_Descriptor,
                                                             FS_Descriptor>(
      *this);
}
    ...
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
      for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
        for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
          ...
          T advection = u[0] * (util::greaterThan(u[0], (T)0)
                                    ? (phiC - phiW)
                                    : (util::lessThan(u[0], (T)0)
                                           ? (phiE - phiC)
                                           : (T)0.5 * (phiE - phiW))) +
                        u[1] * (util::greaterThan(u[1], (T)0)
                                    ? (phiC - phiS)
                                    : (util::lessThan(u[1], (T)0)
                                           ? (phiN - phiC)
                                           : (T)0.5 * (phiN - phiS))) +
                        u[2] * (util::greaterThan(u[2], (T)0)
                                    ? (phiC - phiB)
                                    : (util::lessThan(u[2], (T)0)
                                           ? (phiT - phiC)
                                           : (T)0.5 * (phiT - phiB)));
...
template <typename T>
void AdvectionDiffusionFd3D<T>::getTypeOfModification(
    std::vector<modif::ModifT> &modified) const {
...
template <typename T>
VariableDiffusivityAdvectionDiffusionFd3D<
    T>::VariableDiffusivityAdvectionDiffusionFd3D(bool upwind_)
    : upwind(upwind_) {}
    ...
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
      for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
        for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
          ...
          T DE =
              (T)0.5 * (DC + D->get(iX + 1 + ofs5.x, iY + ofs5.y, iZ + ofs5.z));
          T DW =
              (T)0.5 * (DC + D->get(iX - 1 + ofs5.x, iY + ofs5.y, iZ + ofs5.z));
          T DN =
              (T)0.5 * (DC + D->get(iX + ofs5.x, iY + 1 + ofs5.y, iZ + ofs5.z));
          T DS =
              (T)0.5 * (DC + D->get(iX + ofs5.x, iY - 1 + ofs5.y, iZ + ofs5.z));
          T DT =
              (T)0.5 * (DC + D->get(iX + ofs5.x, iY + ofs5.y, iZ + 1 + ofs5.z));
          T DB =
              (T)0.5 * (DC + D->get(iX + ofs5.x, iY + ofs5.y, iZ - 1 + ofs5.z));
...
template <typename T>
VariableDiffusivityAdvectionDiffusionFd3D<T>
    *VariableDiffusivityAdvectionDiffusionFd3D<T>::clone() const {
...
MaskedVariableDiffusivityAdvectionDiffusionFd3D<T, ADCellT>::
    MaskedVariableDiffusivityAdvectionDiffusionFd3D(
        ADCellT adCellT_, T defaultValue_, Box3D const &fullClosedDomain_,
        bool xPeriodic_, bool yPeriodic_, bool zPeriodic_, bool upwind_)
    : adCellT(adCellT_),
...
template <typename T, class ADCellT>
void MaskedVariableDiffusivityAdvectionDiffusionFd3D<
    T, ADCellT>::processGenericBlocks(Box3D domain,
        ...
        if (adCellT.isSingular(flagC)) {
          ...
          for (plint dx = -1; dx <= 1; ++dx) {
            ...
            for (plint dy = -1; dy <= 1; ++dy) {
              ...
              for (plint dz = -1; dz <= 1; ++dz) {
                plint nextZ = iZ + dz;
                if (contained(nextX + absOfs.x, nextY + absOfs.y,
                              nextZ + absOfs.z, fullExtendedDomain)) {
                  int nextFlag =
                  ...
                  if (!adCellT.isSingular(nextFlag) &&
                    ...
                    avePhi += phi_tp1->get(nextX + ofs1.x, nextY + ofs1.y,
                                           nextZ + ofs1.z);
                    n++;
                  }
                }
              ...
              for (plint dz = -1; dz <= 1; ++dz) {
                ...
                if (contained(nextX + absOfs.x, nextY + absOfs.y,
                  ...
                  if (!adCellT.isSingular(nextFlag) &&
                      !adCellT.isZeroFlux(nextFlag) &&
                      !adCellT.isFixedFlux(nextFlag, phi, dphi)) {
                    phi = phi_tp1->get(nextX + ofs1.x, nextY + ofs1.y,
                                       nextZ + ofs1.z);
                    dphi = (T)0;
          ...
          T advection = (T)0;
          if (upwind) {
            advection = u[0] * (util::greaterThan(u[0], (T)0)
                                    ? (phiC - phiW)
                                    : (util::lessThan(u[0], (T)0)
                                           ? (phiE - phiC)
                                           : (T)0.5 * (phiE - phiW))) +
            ...
            advection = (T)0.5 * (u[0] * (phiE - phiW) + u[1] * (phiN - phiS) +
                                  u[2] * (phiT - phiB));
          }
...
template <typename T, class ADCellT>
void MaskedVariableDiffusivityAdvectionDiffusionFd3D<T, ADCellT>::
    getTypeOfModification(std::vector<modif::ModifT> &modified) const {
  modified[0] = modif::nothing;          // phi_t
  modified[1] = modif::nothing;          // phi_tp1
  modified[2] = modif::staticVariables;  // result
...
TurbulentDiffusivityFunctional3D<
    T, FluidDescriptor>::TurbulentDiffusivityFunctional3D(T D0_, T Pr_t)
    : D0(D0_) {
  PLB_ASSERT(!util::isZero(Pr_t));
  ...
  for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        ...
        T omega = cell.getDynamics().getDynamicParameter(
...
template <typename T, template <typename U> class FluidDescriptor>
void TurbulentDiffusivityFunctional3D<T, FluidDescriptor>::
    getTypeOfModification(std::vector<modif::ModifT> &modified) const {
...
template <typename T, template <typename U> class FluidDescriptor>
void computeTurbulentDiffusivity(T D0, T Pr_t,
                                 MultiBlockLattice3D<T, FluidDescriptor> &fluid,
                                 MultiScalarField3D<T> &diffusivity,
                                 Box3D domain) {
  applyProcessingFunctional(
      new TurbulentDiffusivityFunctional3D<T, FluidDescriptor>(D0, Pr_t),
      domain, fluid, diffusivity);
}

template <typename T, template <typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T>> computeTurbulentDiffusivity(
    T D0, T Pr_t, MultiBlockLattice3D<T, FluidDescriptor> &fluid,
    Box3D domain) {
  std::auto_ptr<MultiScalarField3D<T>> diffusivity =
...
template <typename T, template <typename U> class FluidDescriptor>
void TurbulentDiffusivityFromFlagFunctional3D<T, FluidDescriptor>::
    processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> blocks) {
...
TurbulentDiffusivityFromFlagFunctional3D<T, FluidDescriptor>::clone() const {
  return new TurbulentDiffusivityFromFlagFunctional3D<T, FluidDescriptor>(
      *this);
}
...
void computeTurbulentDiffusivityFromFlag(
    std::vector<T> const &D0, T Pr_t,
    MultiBlockLattice3D<T, FluidDescriptor> &fluid,
    MultiScalarField3D<int> &flags, MultiScalarField3D<T> &diffusivity,
    Box3D domain) {
  std::vector<MultiBlock3D *> args;
  args.push_back(&fluid);
  ...
  applyProcessingFunctional(
      new TurbulentDiffusivityFromFlagFunctional3D<T, FluidDescriptor>(D0,
                                                                       Pr_t),
      domain, args);
}

template <typename T, template <typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T>> computeTurbulentDiffusivityFromFlag(
    std::vector<T> const &D0, T Pr_t,
    MultiBlockLattice3D<T, FluidDescriptor> &fluid,
    MultiScalarField3D<int> &flags, Box3D domain) {
  std::auto_ptr<MultiScalarField3D<T>> diffusivity =
      generateMultiScalarField<T>(fluid, domain);
  computeTurbulentDiffusivityFromFlag(D0, Pr_t, fluid, flags, *diffusivity,
                                      domain);
  return diffusivity;
}
...
std::auto_ptr<MultiScalarField3D<T>> computeTurbulentDiffusivityFromFlag(
  ...
  return computeTurbulentDiffusivityFromFlag(D0, Pr_t, fluid, flags,
...
template <typename T, template <typename U> class FluidDescriptor>
void TurbulentDiffusivityFromScalarFunctional3D<T, FluidDescriptor>::
    processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D *> blocks) {
  PLB_PRECONDITION(blocks.size() == 3);
...
TurbulentDiffusivityFromScalarFunctional3D<T, FluidDescriptor>::clone() const {
  return new TurbulentDiffusivityFromScalarFunctional3D<T, FluidDescriptor>(
      *this);
}
...
void computeTurbulentDiffusivityFromScalar(
    T Pr_t, MultiBlockLattice3D<T, FluidDescriptor> &fluid,
    MultiScalarField3D<T> &D0, MultiScalarField3D<T> &diffusivity,
    Box3D domain) {
  std::vector<MultiBlock3D *> args;
  args.push_back(&fluid);
  args.push_back(&D0);
  args.push_back(&diffusivity);
  applyProcessingFunctional(
      new TurbulentDiffusivityFromScalarFunctional3D<T, FluidDescriptor>(Pr_t),
      domain, args);
}

template <typename T, template <typename U> class FluidDescriptor>
std::auto_ptr<MultiScalarField3D<T>> computeTurbulentDiffusivityFromScalar(
    T Pr_t, MultiBlockLattice3D<T, FluidDescriptor> &fluid,
    MultiScalarField3D<T> &D0, Box3D domain) {
  std::auto_ptr<MultiScalarField3D<T>> diffusivity =
      generateMultiScalarField<T>(fluid, domain);
...
std::auto_ptr<MultiScalarField3D<T>> computeTurbulentDiffusivityFromScalar(
    T Pr_t, MultiBlockLattice3D<T, FluidDescriptor> &fluid,
    MultiScalarField3D<T> &D0) {
  return computeTurbulentDiffusivityFromScalar(Pr_t, fluid, D0,
                                               fluid.getBoundingBox());
}
...
ArrheniusChemicalReactionCoupling3D<T>::ArrheniusChemicalReactionCoupling3D(
    plint nSpecies_, plint nReactions_, T dt_,
    std::vector<T> const &preExponentialFactors_,
    std::vector<T> const &activationEnergies_,
    std::vector<T> const &reactionEnthalpies_,
    ...
    for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
      for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
        ...
        for (plint iReaction = 0; iReaction < nReactions; iReaction++) {
          r[iReaction] =
              ...
              std::exp(-activationEnergies[iReaction] /
                       (R * theta->get(iX + ofsTheta.x, iY + ofsTheta.y,
                                       iZ + ofsTheta.z)));
          for (plint iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
            r[iReaction] *= std::pow(
                C[iSpecies]->get(iX + ofsC[iSpecies].x, iY + ofsC[iSpecies].y,
                                 iZ + ofsC[iSpecies].z),
        ...
        thermalQ->get(iX + ofsTQ.x, iY + ofsTQ.y, iZ + ofsTQ.z) =
            (thetaQ / VHC->get(iX + ofsVHC.x, iY + ofsVHC.y, iZ + ofsVHC.z)) *
            dt;
        ...
        for (plint iSpecies = 0; iSpecies < nSpecies; iSpecies++) {
          ...
          massQ[iSpecies]->get(iX + ofsMQ[iSpecies].x, iY + ofsMQ[iSpecies].y,
                               iZ + ofsMQ[iSpecies].z) = CQ * dt;
        }
      }
    }
  }
...
template <typename T>
ArrheniusChemicalReactionCoupling3D<T>
    *ArrheniusChemicalReactionCoupling3D<T>::clone() const {
