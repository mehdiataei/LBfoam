...
std::unique_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivity(T D0, T Pr_t, MultiBlockLattice3D<T,FluidDescriptor>& fluid,
        Box3D domain)
{
    std::unique_ptr<MultiScalarField3D<T> > diffusivity = generateMultiScalarField<T>(fluid, domain);
...
template<typename T, template<typename U> class FluidDescriptor>
std::unique_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivityFromFlag(std::vector<T> const& D0, T Pr_t,
...
template<typename T, template<typename U> class FluidDescriptor>
std::unique_ptr<MultiScalarField3D<T> > computeTurbulentDiffusivityFromScalar(T Pr_t,
