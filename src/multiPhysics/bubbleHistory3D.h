...
public:
	...
	~BubbleHistory3D();
	void transition(BubbleMatch3D& bubbleMatch, plint iterationStep, T temperature, T R_s, T p_sat, T dx, T fluidRho, T newBubbleVolumeCorrection = (T)1.,
	...
	void updateBubbleGrowth(MultiScalarField3D<T>& outsideDensity, T rhoEmpty, T alpha = (T) -1, T beta = (T) -1, T gamma = (T) 1);
	void scaleAndIncreaseDensity (T temperature, T R_s, T p_sat, T dx, T fluidRho);
	MultiScalarField3D<plint>* getOldTagMatrix()
	{
		return oldTagMatrix;
	}
	void freeze();
...
public:
	...
	void setDisjoiningPressure(double dp)
	{
		disjoiningPressure = dp;
	}
	...
	double getIncrim() const
	{
		return incrim;
	}
	double setDisjoiningPressureField() const
	{
		return disjoiningPressure;
	}
	bool isFrozen() const
	{
		return frozen;
...
{
public:
	UpdateBubbleGrowth3D(std::map<plint,BubbleInfo3D> const& bubbles_, T rho0_, T alpha_, T beta_, T gamma_);
	virtual void process(Box3D domain, ScalarField3D<plint>& tags,
	                     ScalarField3D<T>& density);
