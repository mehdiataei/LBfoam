    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX) {
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {         
                ...
                if (tag>=0)
                {
                    if (flagMatrix.get(iX+flagOffset.x,iY+flagOffset.y,iZ+flagOffset.z)==freeSurfaceFlag3D::empty) {
                    ...
                    else if (flagMatrix.get(iX+flagOffset.x,iY+flagOffset.y,iZ+flagOffset.z)==freeSurfaceFlag3D::interface) {
                        PLB_ASSERT( tag < (plint)bubbleVolume.size() );
