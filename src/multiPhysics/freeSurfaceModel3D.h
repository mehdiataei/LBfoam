...
class InitializeInterfaceLists3D : public BoxProcessingFunctional3D
{
	virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> atomicBlocks)
	{
		...
		InterfaceLists3D<T,Descriptor>* InterfaceLists = new InterfaceLists3D<T,Descriptor>;
...
template<typename T, template<typename U> class Descriptor>
const int FreeSurfaceFields3D<T,Descriptor>::envelopeWidth = 4; // Necessary when we use height functions to compute the curvature,
...
public:
	...
	Actions3D freeSurfaceActions(std::map<std::string, plint>* ids = 0)
	{
		...
		//setToConstant(group.getScalar<int>("flag"), group.getBoundingBox(), (int)freeSurfaceFlag3D::empty);
