...
public:
    FreeSurfaceAddSurfaceTensionWeberModel3D(T surfaceTension_, T rhoDefault_, T characteristicLength_,
        T criticalWe_, T criticalKn_, bool incompressibleModel_)
        : surfaceTension(surfaceTension_),
...
public:
    FreeSurfaceIniInterfaceToAnyNodes3D(T rhoDefault_, Dynamics<T,Descriptor>* emptyNodeDynamicsTemplate_)
        : rhoDefault(rhoDefault_),
    ...
    FreeSurfaceIniInterfaceToAnyNodes3D(FreeSurfaceIniInterfaceToAnyNodes3D<T,Descriptor> const& rhs)
...
public:
    FreeSurfaceRemoveFalseInterfaceCells3D(T rhoDefault_, Dynamics<T,Descriptor>* emptyNodeDynamicsTemplate_)
        : rhoDefault(rhoDefault_),
          emptyNodeDynamicsTemplate(emptyNodeDynamicsTemplate_)
    { }
    ...
    FreeSurfaceRemoveFalseInterfaceCells3D(FreeSurfaceRemoveFalseInterfaceCells3D<T,Descriptor> const& rhs)
        : rhoDefault(rhs.rhoDefault),
          emptyNodeDynamicsTemplate(rhs.emptyNodeDynamicsTemplate->clone())
    { }
...
/** Input:
  ...
  **/
template<typename T,template<typename U> class Descriptor>
class FreeSurfaceEqualMassExcessReDistributionAndComputationOfLostMass3D : public BoxProcessingFunctional3D {
public:
    FreeSurfaceEqualMassExcessReDistributionAndComputationOfLostMass3D(FreeSurfaceReductionData& reductionData_)
        : reductionData(reductionData_)
    { }
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> atomicBlocks);
    virtual FreeSurfaceEqualMassExcessReDistributionAndComputationOfLostMass3D<T,Descriptor>* clone() const {
...
public:
    FreeSurfaceComputeReductions3D(FreeSurfaceReductionData& reductionData_, MPI_Comm& reductionCommunicator_)
        : reductionData(reductionData_),
          reductionCommunicator(reductionCommunicator_)
    { }
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks);
    virtual void getTypeOfModification(std::vector<modif::ModifT>& modified) const
    {
...
public: 
    ...
    Actions3D freeSurfaceActions(std::map<std::string, plint>* ids = 0, T characteristicLength = (T) 0,
        T criticalWe = (T) 0, T criticalKn = (T) 0)
    {
        Actions3D actions;

        bool useSurfaceTension = !util::isZero(surfaceTension);
        bool useWeberModel = !util::isZero(characteristicLength) && !util::isZero(criticalWe) && !util::isZero(criticalKn);
            ...
            if (!useWeberModel) {
                ...
                actions.addProcessor (
                    new FreeSurfaceAddSurfaceTensionWeberModel3D<T,Descriptor>(
                        surfaceTension, rhoDefault, characteristicLength, criticalWe, criticalKn, incompressibleModel),
                    freeSurfaceBlocks, group.getBoundingBox() );
            }
        }
        actions.addProcessor (
...
public: 
    FreeSurfaceWrapper (
            Group3D& group_,
            Dynamics<T,Descriptor>* dynamics_,
            Dynamics<T,Descriptor>* emptyNodeDynamics_, T rhoDefault_,
            T surfaceTension_, T contactAngle_,
            Array<T,Descriptor<T>::ExternalField::sizeOfForce> force_,
            std::string fluidname_="fluid1", std::string fsPrefix_="",
            std::string rhoBarJprefix_="",  bool hasImmersedWalls_=false)
    ...
    FreeSurfaceWrapper (
            Group3D& group_,
            Dynamics<T,Descriptor>* dynamics_, T rhoDefault_,
            T surfaceTension_, T contactAngle_,
            Array<T,Descriptor<T>::ExternalField::sizeOfForce> force_,
            std::string fluidname_="fluid1", std::string fsPrefix_="",
            std::string rhoBarJprefix_="",  bool hasImmersedWalls_=false)
    ...
    // With this constructor, the fluid lattice is provided separately and is not contained in the group.
    FreeSurfaceWrapper (
            Group3D& group_,
    ...
    void createFreeSurfaceFields()
    {
        ...
        plint gridLevel = (lattice ? lattice->getMultiBlockManagement().getRefinementLevel() :
        ...
        if (group.getNumBlocks()) {
            ...
            MultiScalarField3D<T>* mass = generateMultiScalarField<T>((MultiBlock3D&) *lattice, smallEnvelopeWidth).release();
        ...
        if (hasImmersedWalls) {
            if (!fieldExists(fsPrefix+"ibm_container", smallOrLargeEnvelopeWidth)) {
                group.generateContainer(fsPrefix+"ibm_container", smallOrLargeEnvelopeWidth, gridLevel);
            }
        }
    ...
    // Contrary to the Palabos convention, this function does NOT take ownership of the surfaceTensionField
    // despite the fact that a pointer is passed. The caller is responsible for its memory management.
    void createFreeSurfaceProcessors(plint initialProcessorLevel, bool collideAndStreamAtTheEndOfCycle,
            ...
            MultiScalarField3D<T>* surfaceTensionField)
    {
        ...
        MultiBlockLattice3D<T,Descriptor>& fluidLattice = (lattice ? *lattice : group.getLattice<T,Descriptor>(fluidname));
        ...
        integrateProcessingFunctional (
            new FreeSurfaceMacroscopicWithoutLostMassReDistribution3D<T,Descriptor>(incompressibleModel), 
            group.getBoundingBox(), freeSurfaceArgs, pl );
        ...
        integrateProcessingFunctional (
            new FreeSurfaceIniInterfaceToAnyNodes3D<T,Descriptor>(rhoDefault, emptyNodeDynamics->clone()),
            group.getBoundingBox(), freeSurfaceArgs, pl );
        ...
        integrateProcessingFunctional (
            new FreeSurfaceRemoveFalseInterfaceCells3D<T,Descriptor>(rhoDefault, emptyNodeDynamics->clone()),
            group.getBoundingBox(), freeSurfaceArgs, pl);
        ...
        integrateProcessingFunctional (
            new FreeSurfaceEqualMassExcessReDistributionAndComputationOfLostMass3D<T,Descriptor>(reductionData),
            group.getBoundingBox(), freeSurfaceArgs, pl );
...
#else
            integrateProcessingFunctional (
                new FreeSurfaceComputeSerialReductions3D(reductionData),
                group.getBoundingBox(), args, pl );
#endif
        }
...

        {
            ...
            integrateProcessingFunctional (
                new FreeSurfaceResetReductionData3D(reductionData),
                group.getBoundingBox(), args, pl );
        }
        ...
        if (useForce) {
            ...
            integrateProcessingFunctional (
                forceProcessor,
                group.getBoundingBox(), fluidLattice, blocks, pl );
        }
            ...
            BoxProcessingFunctional3D* forceProcessor, std::vector<MultiBlock3D*> blocks)
    {
        ...
        createFreeSurfaceProcessors(initialProcessorLevel, collideAndStreamAtTheEndOfCycle, forceProcessor, blocks,
                surfaceTensionField);
    }

    void createFreeSurfaceProcessors(plint initialProcessorLevel=0, bool collideAndStreamAtTheEndOfCycle=false,
            MultiScalarField3D<T>* surfaceTensionField=0)
    {
    ...
    template<class VelFunction>
    void createFreeSurfaceProcessorsForImmersedWalls(plint numIBIterations, std::vector<Array<T,3> > const& vertices,
            std::vector<T> const& areas, std::vector<int> const& flags, VelFunction velFunction, int repelInterface,
            ...
            MultiScalarField3D<T>* surfaceTensionField)
    {
        ...
        if (repelInterface == 2) {
            ...
            integrateProcessingFunctional(
                new TemporarilyProtectImmersedWalls3D<T>(),
                group.getBoundingBox(), fluidLattice, tmpProtectionArgs, pl);
        }
        ...
        if (repelInterface == 2) {
            ...
            integrateProcessingFunctional(
                new RemoveProtectionFromImmersedWalls3D<T>(), group.getBoundingBox(),
                fluidLattice, rmProtectionArgs, pl);
        }
            ...
            MultiScalarField3D<T>* surfaceTensionField=0)
    {
        ...
        createFreeSurfaceProcessorsForImmersedWalls<VelFunction>(numIBIterations, vertices, areas, flags, velFunction,
                repelInterface, initialProcessorLevel, collideAndStreamAtTheEndOfCycle, forceProcessor, blocks,
                surfaceTensionField);
    }
    ...
    {
        applyProcessingFunctional (
           new DefaultInitializeFreeSurface3D<T,Descriptor>(dynamics->clone(), emptyNodeDynamics->clone(),
               force, rhoDefault, useConstRho, useZeroMomentum, initializeCell),
    ...
    {
        applyProcessingFunctional (
           new PartiallyDefaultInitializeFreeSurface3D<T,Descriptor>(dynamics->clone(), emptyNodeDynamics->clone(),
               force, rhoDefault, useConstRho, useZeroMomentum, initializeCell),
           group.getBoundingBox(), freeSurfaceArgs );
    }
...
private:
    ...
    {
        SparseBlockStructure3D const& sparseBlockStructure = multiBlock.getMultiBlockManagement().getSparseBlockStructure();
        ThreadAttribution const& threadAttribution = multiBlock.getMultiBlockManagement().getThreadAttribution();
        ...
        (void) MPI_Comm_create(global::mpi().getGlobalCommunicator(), reductionGroup, &reductionCommunicator);
    }
#endif
private:
    Group3D& group;
    MultiBlockLattice3D<T,Descriptor>* lattice; // Pointer to a fluid lattice if not contained in the group.
    Dynamics<T,Descriptor>* dynamics;
