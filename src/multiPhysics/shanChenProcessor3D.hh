...
ShanChenDirichlet3D <T,Descriptor>::ShanChenDirichlet3D (
        T* vel_, std::vector<T*> bcDensities_,
        int direction_, int orientation_ )
    : vel(vel_), bcDensities(bcDensities_),
        ...
        for (plint iY=domain.y0; iY<=domain.y1; ++iY) {
            for (plint iZ=domain.z0; iZ<=domain.z1; ++iZ) {
                for (plint sigma=0; sigma<numSpecies; ++sigma) {
                    ...
                    *cell.getExternal(momentumOffset+direction) = *vel * *cell.getExternal(densityOffset);

                    Array<T,Descriptor<T>::q> fEq;
                    Array<T,3> j; j.resetToZero();
...
template<typename T, template<typename U> class Descriptor>
ShanChenDirichlet3D<T,Descriptor>*
    ShanChenDirichlet3D<T,Descriptor>::clone() const
{
    return new ShanChenDirichlet3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void ShanChenDirichlet3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    // All blocks are modified by the Shan/Chen processor.
    for (pluint iBlock=0; iBlock<modified.size(); ++iBlock) {
        modified[iBlock] = modif::staticVariables;
...
template<typename T, template<typename U> class Descriptor>
ShanChenConvergeZone3D<T,Descriptor>*
    ShanChenConvergeZone3D<T,Descriptor>::clone() const
{
    return new ShanChenConvergeZone3D<T,Descriptor>(*this);
}

template<typename T, template<typename U> class Descriptor>
void ShanChenConvergeZone3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
    // All blocks are modified by the Shan/Chen processor.
    for (pluint iBlock=0; iBlock<modified.size(); ++iBlock) {
        modified[iBlock] = modif::staticVariables;
...
template<typename T, template<typename U> class Descriptor>
ComputeMultiPhaseScalar3D<T,Descriptor>::ComputeMultiPhaseScalar3D(std::vector<T> const& scalarValues_, int nonSolidTag_)
    : scalarValues(scalarValues_),
      nonSolidTag(nonSolidTag_)
{ }

template<typename T, template<typename U> class Descriptor>
void ComputeMultiPhaseScalar3D<T,Descriptor>::processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
{ 
    ...
    for (plint iX = domain.x0; iX <= domain.x1; ++iX) {
        for (plint iY = domain.y0; iY <= domain.y1; ++iY) {
            for (plint iZ = domain.z0; iZ <= domain.z1; ++iZ) {
                ...
                if (tag == nonSolidTag) {
                    plint whichPhase = 0;
                    T maxRho = *lattices[0]->get(iX + ofsL[0].x, iY + ofsL[0].y, iZ + ofsL[0].z).getExternal(densityOffset);
                    for (plint iPhase = 1; iPhase < nPhases; iPhase++) {
                        T locRho = *lattices[iPhase]->
                            get(iX + ofsL[iPhase].x, iY + ofsL[iPhase].y, iZ + ofsL[iPhase].z).
...
template<typename T, template<typename U> class Descriptor>
void ComputeMultiPhaseScalar3D<T,Descriptor>::getTypeOfModification(std::vector<modif::ModifT>& modified) const
{
        ...
        MultiScalarField3D<T>& scalar, std::vector<T> const& scalarValues, int nonSolidTag, Box3D const& domain)
{
    ...
    applyProcessingFunctional(new ComputeMultiPhaseScalar3D<T,Descriptor>(scalarValues, nonSolidTag), domain, args);
}
        ...
        std::vector<T> const& scalarValues, int nonSolidTag, Box3D const& domain)
{
    ...
    std::unique_ptr<MultiScalarField3D<T> > scalar = generateMultiScalarField<T>(*lattices[0], domain);
