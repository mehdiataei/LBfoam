...
struct TwoPhaseFields3D {
    ...
    std::unique_ptr<MultiScalarField3D<T> > computePressure(Box3D domain, bool computeFluid1, bool computeFluid2)
    {
    ...
    std::unique_ptr<MultiTensorField3D<T,3> > computeVelocity(Box3D domain, bool computeFluid1, bool computeFluid2)
    {
