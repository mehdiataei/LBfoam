...
public:
	...
	TwoPhaseProcessorParam3D(std::vector<AtomicBlock3D*>& atomicBlocks)
	{
		...
		//Put the assertion at the usage of InterfaceLists3D, so we can still work with both freeSurfaceProcessorParam and twoPhaseProcessorParam.
...
std::vector<MultiBlock3D*> aggregateTwoPhaseParams (
    ...
    MultiContainerBlock3D& InterfaceLists3D, MultiScalarField3D<T>& curvature,
    MultiScalarField3D<T>& outsideDensity )
{
	...
	aggregation.push_back(&InterfaceLists3D);
...
class TwoPhaseInitializeInterfaceLists3D : public BoxProcessingFunctional3D
{
	virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> atomicBlocks)
	{
		...
		TwoPhaseInterfaceLists<T,Descriptor>* InterfaceLists3D = new TwoPhaseInterfaceLists<T,Descriptor>;
		containerInterfaceLists->setData(InterfaceLists3D);
...
template<typename T, template<typename U> class Descriptor>
const int TwoPhaseFields3D<T,Descriptor>::smallEnvelopeWidth = 4;
