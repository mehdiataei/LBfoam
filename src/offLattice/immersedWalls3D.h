...
template<typename T>
Array<T,3> reduceImmersedTorque(MultiContainerBlock3D& container, Array<T,3> const& center, int reductionFlag = 0)
{
    ...
    MultiContainerBlock3D& container )
{
    ...
    args.push_back(&container);
    applyProcessingFunctional (
        new IndexedImmersedBoundaryIteration3D<T,VelFunction>(velFunction), u.getBoundingBox(), args );
}
...
void recomputeImmersedForce(NormalFunction normalFunction, T omega,
        ...
        bool incompressibleModel)
{
    std::unique_ptr<MultiScalarField3D<T> > rhoBar = generateMultiScalarField<T>(lattice, envelopeWidth);
